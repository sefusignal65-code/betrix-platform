...
export async function validateMetrics(
  ...
  if (metrics.errorRate >= config.errorRate.critical) {
    ...
    result.errors.push(`Error rate (${metrics.errorRate}) exceeds critical threshold (${config.errorRate.critical})`);
  } else if (metrics.errorRate >= config.errorRate.warning) {
    result.warnings.push(`Error rate (${metrics.errorRate}) exceeds warning threshold (${config.errorRate.warning})`);
  }

  // Latency Check
  if (metrics.latencyMs.p95 >= config.latencyMs.p95.critical) {
    result.isHealthy = false;
    result.errors.push(`P95 latency (${metrics.latencyMs.p95}ms) exceeds critical threshold (${config.latencyMs.p95.critical}ms)`);
  } else if (metrics.latencyMs.p95 >= config.latencyMs.p95.warning) {
    result.warnings.push(`P95 latency (${metrics.latencyMs.p95}ms) exceeds warning threshold (${config.latencyMs.p95.warning}ms)`);
  }

  // Quota Usage Check
  if (metrics.quotaUsage >= config.quotaUsage.critical) {
    result.isHealthy = false;
    result.errors.push(`Quota usage (${metrics.quotaUsage * 100}%) exceeds critical threshold (${config.quotaUsage.critical * 100}%)`);
  } else if (metrics.quotaUsage >= config.quotaUsage.warning) {
    result.warnings.push(`Quota usage (${metrics.quotaUsage * 100}%) exceeds warning threshold (${config.quotaUsage.warning * 100}%)`);
  }
